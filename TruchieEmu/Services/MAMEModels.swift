...
extension MAME2003PlusDatabase: Decodable {
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: _CodingKeys2003.self)
        self.metadata = try container.decode(MAME2003PlusMetadata.self, forKey: .metadata)
        let rawGames = try container.decode([String: MAME2003PlusGameData].self, forKey: .games)
        self.games = Dictionary(uniqueKeysWithValues:
            rawGames.map { shortName, gameData in
                (shortName, MAME2003PlusEntry(shortName: shortName, gameData: gameData))
            }
        )
    }
}

private enum _CodingKeys2003: String, CodingKey {
    case metadata, games
}
...
struct MAME2003PlusMetadata: Codable {
    ...
    let generatedAt: String
    let totalEntries: Int
    let runnableGames: Int
    let biosEntries: Int
    let unplayableEntries: Int
...
struct MAME2003PlusEntry: Identifiable {
    ...
    let cpuClock: Double?
    let audio: [String]?
    let soundChannels: Int?
    let driverStatus: String?
    let driverColor: String?
    let driverSound: String?
    
    init(shortName: String, gameData: MAME2003PlusGameData) {
        self.shortName = shortName
        self.description = gameData.description
        self.year = gameData.year
        self.manufacturer = gameData.manufacturer
        self.runnable = gameData.runnable
        self.isBIOS = gameData.isBIOS
        self.cloneOf = gameData.cloneOf
        self.romOf = gameData.romOf
        self.sampleOf = gameData.sampleOf
        self.mergedROMs = gameData.mergedROMs
        self.players = gameData.players
        self.control = gameData.control
        ...
        self.refreshRate = gameData.refreshRate
        self.cpu = gameData.cpu
        self.cpuClock = gameData.cpuClock
        self.audio = gameData.audio
        self.soundChannels = gameData.soundChannels
        self.driverStatus = gameData.driverStatus
        self.driverColor = gameData.driverColor
        self.driverSound = gameData.driverSound
    }
    
    var needsNeoGeoBIOS: Bool {
        romOf?.lowercased() == "neogeo" ||
        cloneOf?.lowercased() == "neogeo" ||
        (mergedROMs?.contains { $0.lowercased().contains("neogeo") } ?? false)
    }

...
final class MAMEImportService: ObservableObject {
    ...
    /// In-memory lookup dictionary for O(1) ROM identification.
    /// Loaded once from bundled JSON, then used for fast lookups.
    ...
    /// Uses a static shared dictionary for thread-safe access.
    nonisolated(unsafe) private static var _sharedLookupDict: [String: MAMELookupEntry] = [:]
    nonisolated(unsafe) private static var _isLoaded = false
    
    /// Non-isolated lookup method for use from actors (ROMScanner).
    nonisolated static func lookup(shortName: String) -> MAMELookupEntry? {
        return _sharedLookupDict[shortName.lowercased()]
    }
    
    /// Internal method to populate the static lookup dictionary.
    static func _populateLookupDictionary(_ dict: [String: MAMELookupEntry]) {
        _sharedLookupDict = dict
        _isLoaded = true
    }
    
    static func isLookupLoaded() -> Bool {
        return _isLoaded
    }
    ...
    /// Returns nil if not found in the database.
    func lookup(shortName: String) -> MAMELookupEntry? {
        if !isDictionaryLoaded {
            // Synchronously load if not yet loaded (shouldn't happen in normal flow)
            // In production, loadLookupDictionary() should be called at app launch
            return nil
        }
        return lookupDictionary[shortName.lowercased()]
    }
    
    /// Check if a shortname exists in the database.
    func exists(shortName: String) -> Bool {
        return lookupDictionary[shortName.lowercased()] != nil
    }
    
    /// Get all entries that are playable games.
    func playableGameCount() -> Int {
        return lookupDictionary.values.filter { $0.isPlayableGame }.count
    }
    
    /// Get total entries in the lookup dictionary.
    func totalLookupEntries() -> Int {
        return lookupDictionary.count
    }

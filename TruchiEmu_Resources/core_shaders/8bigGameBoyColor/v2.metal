...
// High-entropy hash for grid jitter to prevent moiré patterns
...
fragment float4 fragment8BitGBC(VertexOut in [[stage_in]],
    ...
    float colBleed = frame0.sample(s, float2(uv.x, 0.5)).g * 0.008;
    sCol -= colBleed;
    ...
    // 7. ANTI-MOIRE STOCHASTIC GRID (Section Updated)
    const float3 pCol = float3(0.20, 0.21, 0.18);
    float2 pCoord = uv * gbcRes;
    float2 unitF = fwidth(pCoord);
    
    // Use a much higher frequency jitter to break up the moire grid
    float gridJitter = hardware_gold_noise(pixelIndex, 12.1234);
    float2 pCoordJittered = pCoord + (gridJitter * 0.015); // Micro-jittering the grid location
    
    // Grid Mask with stochastic edge softening
    float2 grid = abs(fract(pCoordJittered) - 0.5);
    // Increased the unitF multiplier to 2.2 to ensure better blending on Retina screens
    float2 gridAA = smoothstep(0.5, 0.5 - unitF * 2.2, grid);
    ...
    float2 shadowAA = smoothstep(0.5, 0.5 - unitF * 2.2, shadowGrid);

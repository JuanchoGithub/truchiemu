...
fragment float4 fragmentCRT(VertexOut in [[stage_in]],
  ...
  const float BEZEL_BLUR_OFFSET  = 3.5;
  ...
  float4 sharpColor;
  if (USE_RADIAL_SOFT) {
    float blurAmount = pow(dist, 3.0) * 0.0008;
  ...
  float2 gamePixelUV = 1.0 / sourceRes;
  float3 bleedResult = sharpColor.rgb;
  if (USE_DITHER_BLEED) {
    bleedResult = (sharpColor.rgb + tex.sample(s, sampleUV - float2(gamePixelUV.x, 0)).rgb + tex.sample(s, sampleUV + float2(gamePixelUV.x, 0)).rgb) * 0.333;
  }

  // --- [ 7. MIXING ] ---
  float luminance = dot(sharpColor.rgb, float3(0.2126, 0.7152, 0.0722));
  float3 finalRGB = mix(sharpColor.rgb, bleedResult, 3.0);
  finalRGB = mix(finalRGB, sharpColor.rgb, 0.5 + (luminance * 0.5));

  // --- [ 8. ANALOG FINISHING ] ---
  finalRGB *= u.colorBoost * 1.1;
  if (USE_WHITE_BALANCE) finalRGB *= TINT_D65;
  if (USE_VIGNETTE) finalRGB *= clamp(pow(1.0 - dot(centered * VIG_STRENGTH, centered * VIG_STRENGTH), VIG_POWER), 0.0, 1.0);

...
fragment float4 fragmentCRT(VertexOut in [[stage_in]],
    ...
    float scanlineIntensity = u.scanlineIntensity > 0 ? u.scanlineIntensity : 0.45; 
    float barrelAmount = u.barrelAmount > 0 ? u.barrelAmount : 0.01;
    float colorBoost = u.colorBoost > 0 ? u.colorBoost : 1.35;
    float bleedAmount = u.bleedAmount > 0 ? u.bleedAmount : 0.35;
    float time = u.time > 0 ? u.time : 0.2;
    float texSizeX = u.texSizeX > 0 ? u.texSizeX : 320.0;
    float texSizeY = u.texSizeY > 0 ? u.texSizeY : 240.0;
    ...
    const float HORIZONTAL_SMEAR = 2.5; 
    ...
    const float SCANLINE_DARK_BOOST = 1.5;

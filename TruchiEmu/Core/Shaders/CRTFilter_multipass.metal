 ...
 * and an adjustable RGB subpixel mask.
 */
...
struct CRTMultipassUniforms {
    ...
    float bleedAmount;       // FIXED: Now actively used
    float texSizeX;
    float texSizeY;
    ...
    // --- NEW ADDITIONS ---
    float phosphorDecay;       // Control for phosphor tail length (0.0 to 1.0)
    float maskPixelSpacingH;   // Horizontal separation between pixels
    float maskPixelSpacingV;   // Vertical separation between pixels
    float maskSubpixelGap;     // Gap between R, G, and B subpixels (0.0 to 0.9)
    float useMask;             // Toggle for the true CRT subpixel mask
...
static inline float3 crt_mp_applyDitherBleed(float3 rgb, float3 leftColor, float3 rightColor, bool active, float bleedAmt) {
...
static inline float3 crt_mp_applyMaskAndBezel(float3 rgb, float2 distortUV, float2 sampleUV, texture2d<float> tex, sampler s, float boost, float texX, float rounding, float glowInt, bool bezel) {
    ...
    float cornerMask = pow(maskEdge.x, 12.0) + pow(maskEdge.y, 12.0);
...
fragment float4 fragmentCRTMultipass(VertexOut in [[stage_in]],
    ...
    float blurAmt = SOFT ? (ctx.distSq * ctx.distSq) * u.softnessAmount : 0.0; // Fixed hardcoded 0.0008
    ...
    // 4. Phosphor Trail Mix (Adjustable Decay)
    float d = saturate(u.phosphorDecay);
    float3 trail = mix(f1, mix(f2, mix(f3, f4, d), d), d);

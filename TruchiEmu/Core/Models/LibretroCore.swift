...
struct CoreVersion: Codable, Hashable, Identifiable {
    ...
    var version: String // e.g. "v0.5.0-nightly-20260428"
    var buildDate: String // Build date from buildbot
    var dylibPath: URL
    var downloadedAt: Date
    var remoteURL: URL?
    var isActive: Bool = false // NEW: is this the current active version

    // Backwards compatibility: compute tag for existing cores
    var tag: String {
        // Simple concatenation of version and build date
...
struct RemoteCoreInfo: Identifiable, Codable {
    ...
    var metadata: CoreMetadata {
        ...
        return LibretroCore.knownCoreMetadata[baseID]
        ?? CoreMetadata(displayName: displayName, version: version ?? "?", description: "Community-maintained libretro core.", recommendation: nil)
    }
}

...
private func fetchAndCacheDat(for system: SystemInfo) async -> [String: GameMetadataSupplement] {
	...
	let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
	let metadatBase = appSupport.appendingPathComponent("TruchiEmu/Metadat", isDirectory: true)
    ...
    private func cacheFilePath(for type: MetadataType, systemID: String) -> URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return appSupport
            .appendingPathComponent("TruchiEmu/Metadat/\(type.rawValue)", isDirectory: true)
